...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($delivery) {
        // Step 4: Get order details from pending_delivery before deletion
        ...
        // Step 7: Mark delivery as completed and move to history
        $update = $db->prepare("UPDATE to_be_delivered SET status = 'delivered' WHERE id = ?");
        $update->execute([$to_be_delivered_id]);

        // Step 8: Update pending_delivery status to 'delivered'
        ...
        // Step 9: Clean up - Remove from active tables
